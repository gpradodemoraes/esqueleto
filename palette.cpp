...
void TTestView::draw() {
	...
	for (int i = 1; i <= 7; i++) // Loop through palette
								 // (6 entries).
	{
		textAttr = static_cast<char>(getColor(i)); // Obtain attribute for
												   // given index.
...
TTestWindow::TTestWindow() : TWindowInit(initFrame), TWindow(TRect(0, 0, TEST_WIDTH, TEST_HEIGHT), 0, wnNoNumber) {
	TRect r = getExtent();
	r.grow(-2, -2);
