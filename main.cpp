...
#include "cmds.h"	 // User defined command set for this
					 // application
#include "main.h"	 // Application class definition
...
TTestApp::TTestApp() : TProgInit(initStatusLine, initMenuBar, initDeskTop) {}
...
TMenuBar *TTestApp::initMenuBar(TRect bounds) {
	...
	TMenuBar *mainMenu =
		new TMenuBar(bounds, new TMenu(*new TMenuItem("~A~bout...", cmAbout, kbAltA, hcNoContext, 0,
...
void TTestApp::paletteView() {
	...
	if (validView(view)) deskTop->insert(view);
}
